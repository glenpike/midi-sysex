...
describe('ReverbControl', () => {
	...
	it('calls the send function if a control is changed	', () => {
		...
		expect(sendSysexMessage).toHaveBeenCalledWith(sliderConfig.controls[0].address, value)
	})
	...
	describe('Rendering when things are not ready', () => {
		it('disabled sliders if not initialised', () => {
			...
			expect(container.querySelectorAll('input[type="range"]:disabled')).toHaveLength(2)
		})
